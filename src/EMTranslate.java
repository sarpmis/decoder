...
/**
 * Given English and Foreign sentence files, prints out the most likely word to
 * word translations using EM alignment algorithm
 *
 * @author Sarp + Dylan
 */
public class EMTranslate {
    ...
    /**
     * @param engFile
     *            the path to english sentences
     * @param forFile
     *            the path to foreign sentences
     * @param iterations
    ...
    public HashMap runEM(String engFile, String forFile, int iterations) {
        ...
        try {
            ...
            while (engScan.hasNextLine() && forScan.hasNextLine()) {
                ...
                String eng ="";
                char[] engChars = ("NULL " + engScan.nextLine()).toLowerCase().toCharArray();
                for(char c:engChars) {
                ...
                String[] engLst = eng.toLowerCase().split("\\s+");

                String forn ="";
                char[] fornChars = forScan.nextLine().toLowerCase().toCharArray();
                for(char c:fornChars) {
                ...
                String[] fornLst = forn.toLowerCase().split("\\s+");
     ...
     *
     * @param threshold
     *            the minimum probability to print
     */
    public void printProbabilities(double threshold) {
        List<String> sortedEng = table.keySet().stream().sorted().collect(Collectors.toList());

        for (String engWord : sortedEng) {
            HashMap<String, Double> fs = table.get(engWord);

            // sort words alphabetically
            List<String> sortedForn = fs.keySet().stream().sorted().collect(Collectors.toList());
    ...
    public static void main(String[] args) {
        ...
        em.runEM(args[1], args[0], 10);
        System.out.println("printing to file");
        em.printToFile(.01, "emSpanToEng100kCleanedFinal.txt");
